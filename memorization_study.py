        ...
        for level, text in tree_mode:
            if level == 0:
                ...
                if not current_main:
                    ...
                    main_to_subs.setdefault(current_main, [])
                if not current_sub:
                    if main_to_subs.get(current_main):
                        current_sub = main_to_subs[current_main][-1]
            ...
            if subs:
                cards.append(
                    make_card(
                        study_date=study_date,
                        title=title,
                        question=f"{main}의 소제목 전체를 쓰세요 (쉼표 구분)",
                        answer=", ".join(subs),
                        quiz_type="multi",
                    )
                )
            ...
            if details:
                cards.append(
                    make_card(
                        study_date=study_date,
                        title=title,
                        question=f"{main} > {sub}의 세부항목 전체를 쓰세요 (쉼표 구분)",
                        answer=", ".join(details),
                        quiz_type="multi",
                    )
                )
            ...
            if details:
                cards.append(
                    make_card(
                        study_date=study_date,
                        title=title,
                        question=f"{main}의 세부항목 전체를 쓰세요 (쉼표 구분)",
                        answer=", ".join(details),
                        quiz_type="multi",
                    )
                )
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    input,textarea,button{padding:10px 12px;border-radius:10px;border:1px solid var(--line);font-size:14px}
...
function renderQuiz(){
  ...
  if(c.quiz_type === 'multi') typeName = '다중정답형';
...
function checkAnswer(){
  ...
  if(c.quiz_type === 'multi'){
    const a = parseMulti(userAnswer);
    const b = parseMulti(c.answer);
    ok = a.length === b.length && a.every((v, i) => v === b[i]);
  } else {
    ok = normalize(userAnswer) === normalize(c.answer);
  }
